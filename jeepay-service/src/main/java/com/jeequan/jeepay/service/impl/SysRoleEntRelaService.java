...
public class SysRoleEntRelaService extends ServiceImpl<SysRoleEntRelaMapper, SysRoleEntRela> {
     ...
     * 根据人查询出所有权限集合-运营平台
     */
    public List<SysEntitlement> selectEntsByUserId(Long userId, Byte isAdmin, String sysType) {
        if (isAdmin == CS.YES || sysType.equals(CS.SYS_TYPE.MCH)) {
            return sysEntitlementService.list(SysEntitlement.gw().eq(SysEntitlement::getSysType, sysType));
        } else {
            return baseMapper.selectEntsByUserId(userId, sysType, null);
        }
    }
     ...
     * 根据人查询出所有权限集合-商户系统
     */
    public List<SysEntitlement> selectEntsByUserId(Long userId, Byte isAdmin) {
        if (isAdmin == CS.YES) {
            return sysEntitlementService.list(SysEntitlement.gw().eq(SysEntitlement::getSysType, CS.SYS_TYPE.MCH).eq(SysEntitlement::getState, CS.PUB_USABLE));
        } else {
            return baseMapper.selectEntsByUserId(userId, CS.SYS_TYPE.MCH, CS.PUB_USABLE);
        }
    }
