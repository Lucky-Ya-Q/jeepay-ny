...
public class MchInfoController extends CommonCtrl {
    ...
    @PreAuthorize("hasAuthority('ENT_MCH_INFO_CODE')")
    @GetMapping("/getCode/{mchNo}")
    @MethodLog(remark = "获取授权码")
    public ApiRes getCode(@PathVariable("mchNo") String mchNo) {
        MchInfo mchInfo = mchInfoService.getById(mchNo);

        if (mchInfo.getState() == CS.NO) {
            return ApiRes.customFail("当前商户状态不可用！");
        }

        SysConfig sysConfig = sysConfigService.getById("mchSiteUrl");
        String code = authService.auth(mchInfo.getInitUserId());
        sysConfig.addExt(CS.CODE, code);
        return ApiRes.ok(sysConfig);
    }
}
