...
public class AuthService {
    ...
    public String auth(Long userId) {
        ...
        if (sysUser.getState() != CS.PUB_USABLE) {
            throw new BizException("初始用户状态不可登录！");
        }

        if (sysUser.getIsAdmin() != CS.YES && sysEntitlementMapper.userHasLeftMenu(sysUser.getSysUserId(), CS.SYS_TYPE.MCH) <= 0) {
            throw new BizException("初始用户未分配任何菜单权限！");
        }

        JeeUserDetails jeeUserDetails = new JeeUserDetails(sysUser, null);

        String cacheKey = CS.getCacheKeyToken(sysUser.getSysUserId(), IdUtil.fastUUID());
        jeeUserDetails.setCacheKey(cacheKey);

        // 返回code
        return JWTUtils.generateToken(new JWTPayload(jeeUserDetails), CS.JWT_SECRET);
    }
}
